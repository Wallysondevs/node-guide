export default function FsEscrita() {
  return (
    <article className="chapter max-w-3xl mx-auto px-6 py-10">
      <div className="text-xs text-muted-foreground mb-2 font-mono">fs · iniciante · 7 min</div>
      <h1>Escrevendo arquivos</h1>
      <div dangerouslySetInnerHTML={{__html: `<p>Escrever arquivo em Node parece trivial — e é, até você precisar garantir atomicidade, lidar com encoding ou descobrir que dois processos estão escrevendo no mesmo arquivo. O módulo <code>node:fs/promises</code> resolve a maioria dos casos com poucas funções.</p>
...
<div class="callout callout-tip"><div class="callout-title">Permissões</div><div>Defina o modo com <code>{ mode: 0o600 }</code> para arquivos sensíveis (segredos). Por padrão o umask do processo decide.</div></div>`}} />
    </article>
  );
}

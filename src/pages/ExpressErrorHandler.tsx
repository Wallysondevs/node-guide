export default function ExpressErrorHandler() {
  return (
    <article className="chapter max-w-3xl mx-auto px-6 py-10">
      <div className="text-xs text-muted-foreground mb-2 font-mono">Express · intermediario · 10 min</div>
      <h1>Error handler</h1>
      <div dangerouslySetInnerHTML={{__html: `<p>Em Express, qualquer middleware com assinatura de <strong>4 argumentos</strong> <code>(err, req, res, next)</code> é tratado como error handler. Ele deve ser o último <code>app.use</code> da pipeline. Erros chegam lá via <code>next(err)</code> ou via <code>throw</code> dentro de wrappers async.</p>
...
<div class="callout callout-tip"><div class="callout-title">Observabilidade</div><div>Plugue o handler em Sentry/Datadog antes do <code>res.status(...)</code> para capturar contexto (user, body, headers) automaticamente.</div></div>`}} />
    </article>
  );
}

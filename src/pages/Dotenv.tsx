export default function Dotenv() {
  return (
    <article className="chapter max-w-3xl mx-auto px-6 py-10">
      <div className="text-xs text-muted-foreground mb-2 font-mono">process/env · iniciante · 6 min</div>
      <h1>Múltiplos .env</h1>
      <div dangerouslySetInnerHTML={{__html: `<p>Arquivos <code>.env</code> carregam variáveis de ambiente para <code>process.env</code> em desenvolvimento local. Em produção, os valores vêm do orquestrador (PaaS, k8s secrets, GitHub Actions). O pacote <code>dotenv</code> cuida do carregamento; Node 20+ tem suporte nativo via <code>--env-file</code>.</p>
...
<div class="callout callout-tip"><div class="callout-title">dotenv-vault e equivalentes</div><div>Para times distribuídos, considere <code>dotenv-vault</code>, <code>doppler</code> ou <code>1Password CLI</code> em vez de compartilhar <code>.env</code> por Slack.</div></div>`}} />
    </article>
  );
}

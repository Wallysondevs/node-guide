export default function ProcessEnv() {
  return (
    <article className="chapter max-w-3xl mx-auto px-6 py-10">
      ...
      <div dangerouslySetInnerHTML={{__html: `<p><code>process.env</code> expõe as variáveis de ambiente do processo. É o canal padrão para configuração externa: URLs de banco, secrets, feature flags, modo de execução.</p>
...
<div class="callout callout-tip"><div class="callout-title">dotenv (legado)</div><div>O pacote <code>dotenv</code> ainda é útil em Node &lt;20.6 ou quando você precisa de múltiplos .env (.env.local, .env.development) com lógica de override. Para projetos novos no Node 20+, use <code>--env-file</code>.</div></div>`}} />
    </article>
  );
}

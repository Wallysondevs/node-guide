export default function PrismaCrud() {
  return (
    <article className="chapter max-w-3xl mx-auto px-6 py-10">
      ...
      <h1>Prisma: CRUD</h1>
      <div dangerouslySetInnerHTML={{__html: `<p>O Prisma Client expõe uma API tipada para todas as tabelas do schema. Cada model vira uma propriedade em <code>prisma</code> com <code>create</code>, <code>findMany</code>, <code>update</code>, <code>delete</code>, <code>upsert</code> etc.</p>
...
<div class="callout callout-warn"><div class="callout-title">Raw queries</div><div>Use <code>prisma.$queryRaw\\\`select * from x where id = \\${id}\\\`</code> com tagged template — interpolação direta com <code>$queryRawUnsafe</code> abre SQL injection.</div></div>`}} />
    </article>
  );
}

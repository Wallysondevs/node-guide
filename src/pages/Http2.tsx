export default function Http2() {
  return (
    <article className="chapter max-w-3xl mx-auto px-6 py-10">
      <div className="text-xs text-muted-foreground mb-2 font-mono">HTTP nativo · avancado · 7 min</div>
      <h1>HTTP/2</h1>
      <div dangerouslySetInnerHTML={{__html: `<p>HTTP/2 multiplexa várias requests em uma única conexão TCP, comprime headers (HPACK) e usa frames binários. Para APIs com muitas chamadas pequenas, reduz latência e overhead. Node tem suporte nativo via <code>node:http2</code>.</p>
...
<div class="callout callout-warn"><div class="callout-title">undici não fala HTTP/2</div><div>O cliente HTTP padrão do Node 18+ (undici, base do <code>fetch</code>) é HTTP/1.1. Para HTTP/2 client, use <code>node:http2</code> ou <code>fetch-h2</code>.</div></div>`}} />
    </article>
  );
}

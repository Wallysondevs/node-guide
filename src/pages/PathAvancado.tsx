export default function PathAvancado() {
  return (
    <article className="chapter max-w-3xl mx-auto px-6 py-10">
      <div className="text-xs text-muted-foreground mb-2 font-mono">path/os/url · intermediario · 7 min</div>
      <h1>path: casos avançados</h1>
      <div dangerouslySetInnerHTML={{__html: `<p>O módulo <code>node:path</code> manipula caminhos de forma cross-platform. Conhecer os métodos avançados evita bugs de Windows/Linux e ataques de path traversal.</p>
...
<div class="callout callout-tip"><div class="callout-title">Dica</div><div>Para listar arquivos com filtro, combine <code>fs.readdir</code> + <code>path.extname</code>. Para padrões mais ricos, use <code>fs.glob</code> (Node 22+) ou <code>fast-glob</code>.</div></div>`}} />
    </article>
  );
}

export default function ExpressSessions() {
  return (
    <article className="chapter max-w-3xl mx-auto px-6 py-10">
      <div className="text-xs text-muted-foreground mb-2 font-mono">Express · intermediario · 11 min</div>
      <h1>Sessions e cookies</h1>
      <div dangerouslySetInnerHTML={{__html: `<p>Sessions são estado server-side identificado por um cookie no cliente. Útil para auth tradicional (server-rendered) e fluxos OAuth. Em JSON APIs puras, JWT costuma vencer; em SSR ou aplicações com SameSite, sessions ainda são a melhor escolha.</p>
...
<div class="callout callout-tip"><div class="callout-title">Alternativa moderna</div><div>Em apps SPA + API, considere cookies httpOnly com tokens curtos + refresh, ou bibliotecas como <code>iron-session</code> (cookie criptografado stateless).</div></div>`}} />
    </article>
  );
}

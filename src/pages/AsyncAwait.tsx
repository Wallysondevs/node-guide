export default function AsyncAwait() {
  return (
    <article className="chapter max-w-3xl mx-auto px-6 py-10">
      <div className="text-xs text-muted-foreground mb-2 font-mono">Async · iniciante · 8 min</div>
      <h1>async/await</h1>
      <div dangerouslySetInnerHTML={{__html: `<p><code>async/await</code> é açúcar sintático sobre Promises. Permite escrever código assíncrono com aparência sequencial, sem perder concorrência. <strong>Toda função <code>async</code> retorna Promise</strong>; um <code>throw</code> dentro dela vira <em>rejection</em>.</p>
...
<div class="callout callout-warn"><div class="callout-title">unhandledRejection</div><div>Em Node 15+, rejeições não tratadas terminam o processo. Sempre use <code>try/catch</code> em handlers HTTP ou wrappers como <code>express-async-errors</code>.</div></div>`}} />
    </article>
  );
}

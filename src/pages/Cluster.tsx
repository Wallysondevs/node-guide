export default function Cluster() {
  return (
    <article className="chapter max-w-3xl mx-auto px-6 py-10">
      ...
      <h1>Cluster</h1>
      <div dangerouslySetInnerHTML={{__html: `<p>Node é single-threaded por padrão. O módulo <code>cluster</code> permite forkar N processos filhos compartilhando a mesma porta — a forma nativa de aproveitar todos os cores de uma máquina sem mudar o código do servidor.</p>
...
<div class="callout callout-warn"><div class="callout-title">worker_threads ≠ cluster</div><div><code>cluster</code> = processos separados (escala HTTP). <code>worker_threads</code> = threads compartilhando memória (CPU pesada num único processo). Não confunda.</div></div>`}} />
    </article>
  );
}

export default function QueueMicrotask() {
  return (
    <article className="chapter max-w-3xl mx-auto px-6 py-10">
      ...
      <div dangerouslySetInnerHTML={{__html: `<p>Microtasks são callbacks que rodam <strong>imediatamente após</strong> a operação atual da pilha terminar e <strong>antes</strong> da próxima fase do event loop (timers, I/O, etc). Em Node há duas filas: <code>process.nextTick</code> e a fila de microtasks padrão (promises + <code>queueMicrotask</code>).</p>
...
<div class="callout callout-tip"><div class="callout-title">nextTick vs queueMicrotask</div><div>Em código de aplicação, prefira <code>queueMicrotask</code> (padrão web). Use <code>process.nextTick</code> apenas em código de baixo nível que precisa rodar antes de qualquer promise pendente.</div></div>`}} />
    </article>
  );
}

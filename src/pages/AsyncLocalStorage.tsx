export default function AsyncLocalStorage() {
  return (
    <article className="chapter max-w-3xl mx-auto px-6 py-10">
      <div className="text-xs text-muted-foreground mb-2 font-mono">Eventos · avancado · 9 min</div>
      <h1>AsyncLocalStorage</h1>
      <div dangerouslySetInnerHTML={{__html: `<p><code>AsyncLocalStorage</code> (ALS) propaga contexto através de cadeias assíncronas — equivalente a <em>thread-local</em> em outras linguagens. É a base de tracing distribuído, request-id e multi-tenant em frameworks modernos.</p>
...
<div class="callout callout-warn"><div class="callout-title">enterWith</div><div>Existe <code>als.enterWith(store)</code> que não exige callback, mas vaza contexto para tudo que rodar depois no mesmo "tick assíncrono". Use só se souber bem o que está fazendo.</div></div>`}} />
    </article>
  );
}

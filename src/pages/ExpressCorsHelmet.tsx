export default function ExpressCorsHelmet() {
  return (
    <article className="chapter max-w-3xl mx-auto px-6 py-10">
      ...
      <div dangerouslySetInnerHTML={{__html: `<p>Toda API HTTP em produção precisa de três camadas de middleware antes de qualquer rota: <strong>CORS</strong> para permitir browsers, <strong>helmet</strong> para hardening de headers e <strong>compression</strong> para reduzir payloads. São linhas curtas com impacto enorme em segurança e performance.</p>
...
<div class="callout callout-tip"><div class="callout-title">Rate limiting</div><div>Combine com <code>express-rate-limit</code> para bloquear abuso. Use <code>trust proxy</code> se atrás de load balancer.</div></div>`}} />
    </article>
  );
}
